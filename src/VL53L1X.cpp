...
constexpr uint8_t VL51L1X_DEFAULT_CONFIGURATION[] = {
...
void VL53L1X::setInterruptPolarityHigh() {
	setInterruptPolarity(1);
}

void VL53L1X::setInterruptPolarityLow() {
	setInterruptPolarity(0);
}

void VL53L1X::setInterruptPolarity(uint8_t NewPolarity) {
	uint8_t Temp;
 ...
 */
uint8_t VL53L1X::getInterruptPolarity() {
	uint8_t Temp;
...
bool VL53L1X::checkForDataReady() {
	uint8_t Temp;
	uint8_t IntPol;
	// uint8_t isDataReady;
...
void VL53L1X::setTimingBudgetInMs(eTimingBudget timingBudget) {
	uint16_t DM;
...
eTimingBudget VL53L1X::getTimingBudgetInMs() {
	uint16_t Temp;
...
void VL53L1X::setDistanceModeLong() {
	setDistanceMode(2);
}

void VL53L1X::setDistanceModeShort() {
	setDistanceMode(1);
}

void VL53L1X::setDistanceMode(uint16_t DM) {
	eTimingBudget TB;
...
void VL53L1X::setInterMeasurementInMs(uint16_t InterMeasMs) {
	uint16_t ClockPLL;
	double data;
...
uint16_t VL53L1X::getInterMeasurementInMs() {
	uint16_t ClockPLL;
	uint16_t pIM;
	uint32_t tmp;
...
bool VL53L1X::cmdSerialDataAvailable() {
	char cmdRecvChar;
...
void VL53L1X::calibration(int mode) {
...
void VL53L1X::setXTalk(uint16_t XtalkValue) {
...
void VL53L1X::setDistanceThreshold(uint16_t ThreshLow, uint16_t ThreshHigh, eWindows Window) {
	uint8_t Temp = 0;
...
eWindows VL53L1X::getDistanceThresholdWindow() {
	eWindows window;
	uint8_t tmp;
...
int8_t VL53L1X::calibrateOffset(uint16_t targetDistInMm) {
...
int8_t VL53L1X::calibrateXTalk(uint16_t targetDistInMm) {
...
void VL53L1X::writeByteData(uint16_t index, uint8_t data) {
	i2CWrite(index, &data, 1);
}

void VL53L1X::writeWordData(uint16_t index, uint16_t data) {
	uint8_t buffer[2];
...
void VL53L1X::writeWordData32(uint16_t index, uint32_t data) {
	uint8_t buffer[4];
...
void VL53L1X::readByteData(uint16_t index, uint8_t* data) {
	i2CRead(index, data, 1);
}

void VL53L1X::readWordData(uint16_t index, uint16_t* data) {
...
void VL53L1X::readWordData32(uint16_t index, uint32_t* data) {
...
void VL53L1X::i2CWrite(uint16_t reg, uint8_t* pBuf, uint16_t len) {
	lastOperateStatus = eVL53L1X_WriteRegError;

	int fileDescriptor = open("/dev/i2c-5", O_RDWR);
	if (fileDescriptor < 0) {
		std::cerr << "[i2c-write] Failed to open device: " << strerror(errno) << std::endl;
		return;
	}

	uint8_t writeBuffer[len + 2];
	writeBuffer[0] = (reg >> 8) & 0xFF;
	writeBuffer[1] = reg & 0xFF;
	for (int i = 0; i < len; ++i) {
		writeBuffer[i + 2] = pBuf[i];
	}
	i2c_msg msgs[1] = {{
		.addr = addr,
		.flags = 0,
		.len = static_cast<uint16_t>(len + 2),
		.buf = writeBuffer,
	}};
	...
	if (result < 0) {
		std::cerr << "[i2c-write] error: " << result << " errno: " << strerror(errno) << std::endl;
		return;
	}
	lastOperateStatus = eVL53L1X_ok;
}

void VL53L1X::i2CRead(uint16_t reg, uint8_t* pBuf, uint16_t len) {
	lastOperateStatus = eVL53L1X_ReadRegError;

	int fileDescriptor = open("/dev/i2c-5", O_RDWR);
	if (fileDescriptor < 0) {
		std::cerr << "[i2c-read] Failed to open device: " << strerror(errno) << std::endl;
		return;
	}
	...
	if (result < 0 || result2 < 0) {
		std::cerr << "[i2c-read] error when reading register: " << result << " errno: " << strerror(errno) << std::endl;
		return;
	}
